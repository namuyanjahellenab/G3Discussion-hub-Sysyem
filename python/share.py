...
def _build_share_links(topic_id: Any, title: str, reply_count: int, base_url: str) -> dict[str, Any]:
    # Builds a shareable link and message for a topic, then generates
    # platform specific URLs (WhatsApp, Twitter, Facebook, email) that each
    # encode the same title/reply count text and link, using each
    # platform's own URL scheme for pre filled share content.
